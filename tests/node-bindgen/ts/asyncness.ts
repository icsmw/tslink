...
import { StructAsyncMethods, AsyncErrorA } from "binding";

const tests = new Group("Async Methods Tests");
const struct = new StructAsyncMethods();

{
    ...
    struct
        ...
        .catch((err: Error) => {
            test.fail(
                `Function getData() of StructAsyncMethods returns error: ${err.message}`
            );
        })
        .finally(() => {
...

{
    ...
    struct
        ...
        .catch((err: Error) => {
            test.fail(
                `Function getDataA() of StructAsyncMethods returns error: ${err.message}`
            );
        })
        .finally(() => {
            clearTimeout(timeout);
...

{
    ...
    struct
        ...
        .catch((err: Error) => {
            test.fail(
                `Function testOfErrorSupportOk() of StructAsyncMethods returns error: ${err.message}`
            );
        })
        .finally(() => {
            clearTimeout(timeout);
...

{
    ...
    struct
        ...
        .catch((err: Error & { err: AsyncErrorA }) => {
